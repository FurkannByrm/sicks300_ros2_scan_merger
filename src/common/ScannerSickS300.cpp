...
bool ScannerSickS300::open(const char* pcPort, int iBaudRate, int iScanId=7)
{
	...
	_serial_io.setBaudRate(iBaudRate);
	_serial_io.setDeviceName(pcPort);
	_serial_io.setBufferSize(READ_BUF_SIZE - 10 , WRITE_BUF_SIZE -10 );
	_serial_io.setHandshake(HandshakeFlags::HS_NONE);
	_serial_io.setMultiplier(m_dBaudMult);
	bRetSerial = _serial_io.openIO();
	_serial_io.setTimeout(0.0);
	_serial_io.setFormat(8, ParityFlags::PA_NONE, StopBits::SB_ONE);
...
bool ScannerSickS300::getScan(std::vector<double> &vdDistanceM, std::vector<double> &vdAngleRAD, std::vector<double> &vdIntensityAU, unsigned int &iTimestamp, unsigned int &iTimeNow, const bool debug)
{
	...
	iNumRead2 = _serial_io.readBlocking((char*)m_ReadBuf+m_actualBufferSize, SCANNER_S300_READ_BUF_SIZE-2-m_actualBufferSize);
